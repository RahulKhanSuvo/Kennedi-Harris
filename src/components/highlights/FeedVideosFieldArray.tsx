...
interface FeedVideosFieldArrayProps {
...
export function FeedVideosFieldArray({
  fields,
  register,
  setValue,
  control,
  ...
  const handleFileChange = (
    ...
    if (file) {
      setValue(`feedVideos.${index}.feedVideos`, file);
    }
  };
    ...
    <div className="pt-6 border-t border-white/5 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-condensed uppercase tracking-wider mt-0.5">
            Clips or highlights with standalone title banners
          </p>
        </div>
        <Button
          type="button"
          onClick={() =>
            append({ title: "", feedVideos: undefined, video_url: "" })
          }
          disabled={isPending}
      ...
      <div className="space-y-3">
        {fields.map((field, index) => {
          const watchFile = control._formValues.feedVideos?.[index]?.feedVideos;
          ...
          return (
            ...
            >
              ...
              <div className="md:col-span-7 space-y-1">
                ...
                <Input
                  placeholder="e.g. Defensive Highlights"
                  {...register(`feedVideos.${index}.title`)}
                  disabled={isPending}
