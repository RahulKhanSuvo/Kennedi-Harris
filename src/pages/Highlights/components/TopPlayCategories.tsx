        ...
        <div className="w-full flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-6 text-left">
          <div className="flex flex-col gap-2">
            <h3 className="font-display text-3xl sm:text-4xl lg:text-5xl text-white tracking-tighter uppercase leading-none m-0">
              TOP <span className="text-kh-pink">FEED</span>
            </h3>
          </div>
          ...
          {isLoading || !FeedVideoData || FeedVideoData.length === 0
            ? Array.from({ length: 4 }).map((_, index) => (
                <div
                  ...
                  className="flex-auto w-full sm:w-1/2 lg:basis-1/4 bg-zinc-950 h-[250px] lg:h-[480px] p-6 sm:p-8 flex flex-col justify-between relative overflow-hidden border border-white/5"
                >
            ...
            : FeedVideoData.map((tape, index) => {
                ...
                return (
                  <div
                    ...
                    /* THE "WHOLE WORLD" NEXT LINE FILL EFFECT:
                    - 'lg:basis-1/4': Sets target allocation to max 4 items per row.
                    - 'flex-auto': Allows remaining items on the next row line to aggressively expand and fill the whole space!
                    - 'lg:grow-[2]' vs 'lg:grow-[1]': Powers the smooth wide expansion animation when hovered.
                  */
                    className={`relative cursor-pointer transition-all duration-700 ease-out flex flex-col justify-between p-6 sm:p-8 overflow-hidden group border border-white/5 
                    w-full sm:w-1/2 lg:basis-1/4 h-[300px] lg:h-[480px] flex-auto
                    ${
                      isActive
                        ? "lg:grow-[2.5] bg-zinc-900"
                        : "lg:grow bg-black"
                    }`}
                  >
                    ...
                    <div className="relative z-10 text-left flex flex-col items-start w-full mt-4 lg:mt-0">
                      <h4 className="font-display text-xl sm:text-2xl lg:text-4xl font-black tracking-tighter text-white leading-none uppercase mt-1 mb-2 lg:mb-3 truncate w-full">
                        {tape.title}
                      </h4>
                    </div>
            ...
            <div className="flex items-center justify-between p-4 bg-zinc-900/60 border-b border-white/5 text-white">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm tracking-wide uppercase font-bold">
                  Streaming Feed Track // {modalVideo.title}
                </span>
              </div>
              <button
                onClick={() => setModalVideo(null)}
